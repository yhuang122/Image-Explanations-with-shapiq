...
def gradeclip_text(c, q_outs, k_outs, lastvs, attn_outputs, eos_position, withksim=True):
...
def gradeclip(c, q_out, k_out, v, att_output, map_size, withksim=True):
    D = k_out.shape[-1]
    ## gradient on last attention output
    grad = torch.autograd.grad(
        c,
    ...
    if withksim:
        ...
        # emap_lastv = (grad_cls * v[1:,0,:] * cosine_qk[:,None]).detach().sum(-1)
    else:
        emap_lastv = F.relu_((grad_cls * v[1:,0,:]).detach().sum(-1)) 
        # emap_lastv = (grad_cls * v[1:,0,:]).detach().sum(-1)
