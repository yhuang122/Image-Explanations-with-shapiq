...
def plot_image_and_text_together(
    ...
    *,
    ...
    Args:
        ...
        segmenter: Optional segmenter with a ``_label_map`` ``(H, W)`` integer tensor
            for irregular superpixel layouts. When provided, heatmap centroids and
            interaction anchors are derived from the actual superpixel regions instead
            of a square grid.
    ...
    if plot_heatmap:
        if label_map is not None:
            # Irregular layout: build heatmap from label_map
            H, W = img.shape[:2]
            heatmap_arr = np.zeros((H, W, 4), dtype=np.float32)
            for k in range(n_image_players):
                c_rgba = colors[(k,)]
                heatmap_arr[label_map == k] = (*c_rgba[:3], min(c_rgba[3], 1.0))
            heatmap = Image.fromarray((heatmap_arr * 255).astype(np.uint8))
        else:
    ...
    if label_map is not None:
        ...
        for k in range(n_image_players):
            ...
            if len(ys):
                ...
                cx, cy = 0.0, 0.0
            x = left + (cx / W) * image_width
            y = bottom + (1.0 - cy / H) * image_height
            if debug:
                print("image player:", {k}, "centroid:", cx, cy, "x:", x, "y:", y)
    ...
    for i, (word, dim) in enumerate(zip(text, word_dimensions)):
        # Skip empty padding tokens (prevents ghost interaction curves)
        if not word.strip():
            x_pos += dim[0] + word_spacing
            continue
    ...
    if iv.max_order >= 2 and plot_interactions:
        ...
        all_2nd_order = sort_interactions(iv_second_order, reverse=True, sort_by_abs=sort_by_abs)

        # Bipartite filter: only keep Image ↔ Text cross-modal interactions
        cross_modal_interactions = []
        for interaction_data in all_2nd_order:
            interaction = interaction_data[0]
            if len(interaction) == 2:
                p1, p2 = interaction
                if (p1 < n_image_players) != (p2 < n_image_players):
        ...
        for interaction in top_interactions:
            ...
            # Safety: skip interactions referencing skipped padding tokens
