...
Algorithm:
    ...
    2. generate_masks (per batch): map region-level coalitions to pixel-level
...
class GradientGuidedSegmenter(Segmenter):
    ...
    def __init__(
        ...
        self._saliency: np.ndarray | None = None

        if model is None or processor is None or image is None or not text:
            raise ValueError(
        ...
        self._label_map_by_device: dict[torch.device, torch.Tensor] = {
            torch.device("cpu"): self._label_map
        }
    ...
    def get_layout(self) -> SpatialLayout:
        """Produce spatial layout describing player-to-pixel mapping."""
        return self._layout
